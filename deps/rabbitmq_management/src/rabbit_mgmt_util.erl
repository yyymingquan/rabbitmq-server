...
%% CVE-2025-50200: Set 404 response body without sending reply,
%% to avoid Cowboy logging Authorization headers on double reply errors.
-spec set_resp_not_found(binary(), cowboy_req:req()) -> cowboy_req:req().
set_resp_not_found(NotFoundBin, ReqData) ->
  ErrorMessage = case vhost(ReqData) of
                   not_found ->
                     <<"vhost_not_found">>;
                   _ ->
                     NotFoundBin
